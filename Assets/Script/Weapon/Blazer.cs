...
public class Blazer : WeaponBase
{
    [SerializeField]
    protected LaserBullet m_bullet;
    ...
    [SerializeField]
    private float m_reloadTime = 5;

    [SerializeField]
    private float m_shotInterval = 0.0f;

    [SerializeField]
    private int m_bulletMax = 999;

    [SerializeField]
    private float m_scopeRatio = 2;
    ...
    public override void Shot() {
        if (m_canShot) {
            //弾の生成
            LaserBullet bullet= Instantiate(m_bullet, m_muzzleTransform.position, m_muzzleTransform.rotation);
            bullet.SetShotRange(m_muzzleTransform.position, m_muzzleTransform.forward * 1000);
